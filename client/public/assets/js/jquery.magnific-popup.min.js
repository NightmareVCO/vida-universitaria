...
!((a) => {
	...
	(t.prototype = {
		...
		_hasScrollBar: (a) => (b.isIE7 ? d.height() : document.body.scrollHeight) > (a || v.height()),
		_setFocus: () => {
			(b.st.focus ? b.content.find(b.st.focus).eq(0) : b.wrap).focus();
		},
		_onFocusIn: (c) =>
			c.target === b.wrap[0] || a.contains(b.wrap[0], c.target) ? void 0 : (b._setFocus(), !1),
		_parseMarkup: (b, c, d) => {
			var e;
		...
		(a.magnificPopup = {
			...
			close: () => a.magnificPopup.instance && a.magnificPopup.instance.close(),
			...
			if ('string' == typeof c)
				if ('open' === c) {
					var e,
						...
						g = Number.parseInt(arguments[1], 10) || 0;
					f.items
						? (e = f.items[g])
		...
		proto: {
			...
			resizeImage: () => {
				...
				if (a && a.img && b.st.image.verticalFit) {
					...
					b.isLowIE &&
						(c =
							Number.parseInt(a.img.css('padding-top'), 10) +
							Number.parseInt(a.img.css('padding-bottom'), 10)),
	...
	var N,
		O = () => (void 0 === N && (N = void 0 !== document.createElement('p').style.MozTransform), N);
	a.magnificPopup.registerModule('zoom', {
		options: {
			enabled: !1,
			easing: 'ease-in-out',
			duration: 300,
			opener: (a) => (a.is('img') ? a : a.find('img')),
		},
		proto: {
			initZoom: () => {
				var a,
			...
			_allowZoom: () => 'image' === b.currItem.type,
			_getItemToZoom: () => (b.currItem.hasSize ? b.currItem.img : !1),
			_getOffset: (c) => {
				var d;
				d = c ? b.currItem.img : b.st.zoom.opener(b.currItem.el || b.currItem);
				var e = d.offset(),
					f = Number.parseInt(d.css('padding-top'), 10),
					g = Number.parseInt(d.css('padding-bottom'), 10);
	...
	var S = (a) => {
		...
		T = (a, b, c) => a.replace(/%curr%/gi, b + 1).replace(/%total%/gi, c);
						...
						? ((f += ' mfp-gallery'),
							w(m + e, () => {
								c.navigateByImgClick &&
									b.wrap.on('click' + e, '.mfp-img', () =>
										b.items.length > 1 ? (b.next(), !1) : void 0,
									),
									d.on('keydown' + e, (a) => {
										37 === a.keyCode ? b.prev() : 39 === a.keyCode && b.next();
									});
							}),
							w('UpdateStatus' + e, (a, c) => {
	...
	a.magnificPopup.registerModule(U, {
		options: {
			replaceSrc: (a) => a.src.replace(/\.\w+$/, (a) => '@2x' + a),
			ratio: 1,
		},
		proto: {
			initRetina: () => {
				if (window.devicePixelRatio > 1) {
					...
					(c = isNaN(c) ? c() : c),
						c > 1 &&
							(w('ImageHasSize.' + U, (a, b) => {
								b.img.css({
									'max-width': b.img[0].naturalWidth / c,
									width: '100%',
								});
							}),
							w('ElementParse.' + U, (b, d) => {
