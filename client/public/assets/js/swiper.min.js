...
!((e, t) => {
		...
		outerWidth: function (e) {
			if (this.length > 0) {
				if (e) {
					...
					return (
						this[0].offsetWidth +
						Number.parseFloat(e.getPropertyValue("margin-right")) +
						Number.parseFloat(e.getPropertyValue("margin-left"))
					);
				}
		...
		outerHeight: function (e) {
			if (this.length > 0) {
				if (e) {
					...
					return (
						this[0].offsetHeight +
						Number.parseFloat(e.getPropertyValue("margin-top")) +
						Number.parseFloat(e.getPropertyValue("margin-bottom"))
					);
				}
		...
		return (
			...
			"x" === t &&
				(i = s.WebKitCSSMatrix
					? n.m41
					: 16 === a.length
						? Number.parseFloat(a[12])
						: Number.parseFloat(a[4])),
			"y" === t &&
				(i = s.WebKitCSSMatrix
					? n.m42
					: 16 === a.length
						? Number.parseFloat(a[13])
						: Number.parseFloat(a[5])),
			i || 0
		);
	}
	function m(e) {
	...
	var C = {
		...
		onAny(e, t) {
			...
			return (
				this.eventsAnyListeners.indexOf(e) < 0 && this.eventsAnyListeners[a](e),
				this
			);
		},
		offAny(e) {
			if (!this.eventsAnyListeners) return this;
			const s = this.eventsAnyListeners.indexOf(e);
			return s >= 0 && this.eventsAnyListeners.splice(s, 1), this;
		},
		off(e, t) {
			return this.eventsListeners
				? (e.split(" ").forEach((e) => {
						void 0 === t
							? (this.eventsListeners[e] = [])
							: this.eventsListeners[e] &&
								this.eventsListeners[e].forEach((a, i) => {
									(a === t || (a.__emitterProxy && a.__emitterProxy === t)) &&
										this.eventsListeners[e].splice(i, 1);
								});
					}),
					this)
				: this;
		...
		emit(...e) {
			if (!this.eventsListeners) return this;
			let s, a, i;
			"string" == typeof e[0] || Array.isArray(e[0])
				? ((s = e[0]), (a = e.slice(1, e.length)), (i = this))
				: ((s = e[0].events), (a = e[0].data), (i = e[0].context || this)),
				a.unshift(i);
			return (
				(Array.isArray(s) ? s : s.split(" ")).forEach((e) => {
					this.eventsAnyListeners &&
						this.eventsAnyListeners.length &&
						this.eventsAnyListeners.forEach((t) => {
							t.apply(i, [e, ...a]);
						}),
						this.eventsListeners &&
							this.eventsListeners[e] &&
							this.eventsListeners[e].forEach((e) => {
								e.apply(i, a);
							});
				}),
				this
	...
	function S(e) {
		const s = a(),
			...
			{ params: l, touches: o, enabled: c } = this;
		if (!c) return;
		if (this.animating && l.preventInteractionOnTransition) return;
		!this.animating && l.cssMode && l.loop && this.loopFix();
		let p = e;
		p.originalEvent && (p = p.originalEvent);
		let h = d(p.target);
		if ("wrapper" === l.touchEventsTarget && !h.closest(this.wrapperEl).length)
			return;
		if (
		...
		if (
			...
			const a = e && this.allowTouchMove && l.touchStartPreventDefault;
			(!l.touchStartForcePreventDefault && !a) ||
				h[0].isContentEditable ||
				p.preventDefault();
		}
		this.emit("touchStart", p);
	}
	function M(e) {
		const t = a(),
			i = this.touchEventsData,
			{ params: r, touches: n, rtlTranslate: l, enabled: o } = this;
		if (!o) return;
		let c = e;
		...
		if (
			...
			Math.sqrt(f ** 2 + g ** 2) < this.params.threshold
		)
			return;
				...
				? ((w = !1),
					r.resistance &&
						(i.currentTranslate =
							this.minTranslate() -
							1 +
							(-this.minTranslate() + i.startTranslate + v) ** b))
				: v < 0 &&
					...
					r.resistance &&
						(i.currentTranslate =
							this.maxTranslate() +
							1 -
							(this.maxTranslate() - i.startTranslate - v) ** b)),
	...
	function P(e) {
		...
		if (
			...
			s.allowTouchCallbacks && this.emit("touchEnd", o),
			(s.allowTouchCallbacks = !1),
			!s.isTouched)
		)
			return (
				s.isMoved && a.grabCursor && this.setGrabCursor(!1),
				(s.isMoved = !1),
		...
		if (
			(this.allowClick &&
				(this.updateClickedSlide(o),
				this.emit("tap click", o),
				c < 300 &&
					d - s.lastClickTime < 300 &&
					this.emit("doubleTap doubleClick", o)),
			(s.lastClickTime = u()),
			p(() => {
		...
		if (
			...
			return;
		if (this.params.freeMode && a.freeMode.enabled)
			return void this.freeMode.onTouchEnd({ currentPos: h });
		let m = 0,
			f = this.slidesSizesGrid[0];
		for (
			let e = 0;
			e < n.length;
		...
		if (c > a.longSwipesMs) {
			if (!a.longSwipes) return void this.slideTo(this.activeIndex);
			"next" === this.swipeDirection &&
				(g >= a.longSwipesRatio ? this.slideTo(m + v) : this.slideTo(m)),
				"prev" === this.swipeDirection &&
					(g > 1 - a.longSwipesRatio ? this.slideTo(m + v) : this.slideTo(m));
		} else {
			if (!a.shortSwipes) return void this.slideTo(this.activeIndex);
			this.navigation &&
			(o.target === this.navigation.nextEl ||
				o.target === this.navigation.prevEl)
				? o.target === this.navigation.nextEl
					? this.slideTo(m + v)
					: this.slideTo(m)
				: ("next" === this.swipeDirection && this.slideTo(m + v),
					"prev" === this.swipeDirection && this.slideTo(m));
		}
	}
	function k() {
		const { params: t, el: s } = this;
		if (s && 0 === s.offsetWidth) return;
		t.breakpoints && this.setBreakpoint();
		const { allowSlideNext: a, allowSlidePrev: i, snapGrid: r } = this;
	...
	function O() {
		const { wrapperEl: t, rtlTranslate: s, enabled: a } = this;
		if (!a) return;
		let i;
		...
		(i = 0 === r ? 0 : (this.translate - this.minTranslate()) / r),
			i !== this.progress &&
				this.updateProgress(s ? -this.translate : this.translate),
	...
	const B = {
			...
			update: {
				updateSize: function () {
					...
					(t =
						...
						(0 === t && this.isHorizontal()) ||
							...
							((t =
								t -
								Number.parseInt(a.css("padding-left") || 0, 10) -
								Number.parseInt(a.css("padding-right") || 0, 10)),
							(s =
								s -
								Number.parseInt(a.css("padding-top") || 0, 10) -
								Number.parseInt(a.css("padding-bottom") || 0, 10)),
							Number.isNaN(t) && (t = 0),
							Number.isNaN(s) && (s = 0),
							Object.assign(this, {
								width: t,
				...
				updateSlides: function () {
					...
					function s(e, s) {
						return Number.parseFloat(e.getPropertyValue(t(s)) || 0);
					}
					const a = e.params,
					...
					"string" == typeof x &&
						x.indexOf("%") >= 0 &&
						(x = (Number.parseFloat(x.replace("%", "")) / 100) * r),
						(e.virtualSize = -x),
						n
				...
				updateAutoHeight: function (e) {
					...
					if (
						...
						if (this.params.centeredSlides)
							this.visibleSlides.each((e) => {
								s.push(e);
							});
						else
							for (i = 0; i < Math.ceil(this.params.slidesPerView); i += 1) {
								const e = this.activeIndex + i;
								if (e > this.slides.length && !a) break;
								s.push(n(e));
							}
					else s.push(n(this.activeIndex));
				...
				updateSlidesOffset: function () {
					...
					for (let s = 0; s < t.length; s += 1)
						t[s].swiperSlideOffset = this.isHorizontal()
							? t[s].offsetLeft
							: t[s].offsetTop;
				},
				updateSlidesProgress: function (e = (this && this.translate) || 0) {
					const s = this.params,
						{ slides: a, rtlTranslate: i } = this;
					...
					for (let e = 0; e < a.length; e += 1) {
						...
						const o =
								(r + (s.centeredSlides ? this.minTranslate() : 0) - l) /
								(n.swiperSlideSize + s.spaceBetween),
							d = -(r - l),
							c = d + this.slidesSizesGrid[e];
						((d >= 0 && d < this.size - 1) ||
							(c > 1 && c <= this.size) ||
							(d <= 0 && c >= this.size)) &&
				...
				updateProgress: function (e) {
					if (void 0 === e) {
						const s = this.rtlTranslate ? -1 : 1;
						e = (this && this.translate && this.translate * s) || 0;
					}
					const s = this.params,
						a = this.maxTranslate() - this.minTranslate();
					let { progress: i, isBeginning: r, isEnd: n } = this;
					const l = r,
						o = n;
					0 === a
						? ((i = 0), (r = !0), (n = !0))
						: ((i = (e - this.minTranslate()) / a), (r = i <= 0), (n = i >= 1)),
						Object.assign(this, { progress: i, isBeginning: r, isEnd: n }),
				...
				updateActiveIndex: function (e) {
					const s = this.rtlTranslate ? this.translate : -this.translate,
						{
							slidesGrid: a,
							snapGrid: i,
							params: r,
			...
			transition: {
				...
				transitionStart: function (e = !0, t) {
					...
					a.cssMode ||
						...
						$({ swiper: this, runCallbacks: e, direction: t, step: "Start" }));
				},
				transitionEnd: function (e = !0, t) {
					...
					(this.animating = !1),
						a.cssMode ||
							(this.setTransition(0),
							$({ swiper: this, runCallbacks: e, direction: t, step: "End" }));
				},
			},
			slide: {
				...
				slideToLoop: function (e = 0, t = this.params.speed, s = !0, a) {
					...
					return (
						...
						this.slideTo(r, t, s, a)
					);
				},
				slideNext: function (e = this.params.speed, t = !0, s) {
					const { animating: i, enabled: r, params: n } = this;
					if (!r) return this;
					let l = n.slidesPerGroup;
					"auto" === n.slidesPerView &&
						1 === n.slidesPerGroup &&
						n.slidesPerGroupAuto &&
						(l = Math.max(this.slidesPerViewDynamic("current", !0), 1));
					const o = this.activeIndex < n.slidesPerGroupSkip ? 1 : l;
					if (n.loop) {
				...
				slidePrev: function (e = this.params.speed, t = !0, s) {
					...
					const p = c(o ? this.translate : -this.translate),
						u = n.map((e) => c(e));
				...
				slideToClosest: function (e = this.params.speed, t = !0, s, a = 0.5) {
					...
					const n = Math.min(this.params.slidesPerGroupSkip, r),
						l = n + Math.floor((r - n) / this.params.slidesPerGroup),
						o = this.rtlTranslate ? this.translate : -this.translate;
					if (o >= this.snapGrid[l]) {
						const e = this.snapGrid[l];
						o - e > (this.snapGrid[l + 1] - e) * a &&
							(r += this.params.slidesPerGroup);
					} else {
						const e = this.snapGrid[l - 1];
						o - e <= (this.snapGrid[l] - e) * a &&
							(r -= this.params.slidesPerGroup);
					}
					return (
						(r = Math.max(r, 0)),
				...
				slideToClickedSlide: function () {
					const { params: t, $wrapperEl: s } = this,
						a =
					...
					if (t.loop) {
						...
						(i = Number.parseInt(
							d(this.clickedSlide).attr("data-swiper-slide-index"),
							10,
						)),
							t.centeredSlides
								? r < this.loopedSlides - a / 2 ||
									r > this.slides.length - this.loopedSlides + a / 2
									? (this.loopFix(),
										(r = s
											.children(
				...
				loopCreate: function () {
					...
					"auto" !== s.slidesPerView ||
						...
						(this.loopedSlides = Math.ceil(
							Number.parseFloat(s.loopedSlides || s.slidesPerView, 10),
						)),
						(this.loopedSlides += s.loopAdditionalSlides),
						this.loopedSlides > r.length && (this.loopedSlides = r.length);
					const n = [],
						l = [];
					r.each((t, s) => {
						const a = d(t);
						s < this.loopedSlides && l.push(t),
							s < r.length && s >= r.length - this.loopedSlides && n.push(t),
			...
			breakpoints: {
				setBreakpoint: function () {
					...
					const l = this.getBreakpoint(n, this.params.breakpointsBase, this.el);
					if (!l || this.currentBreakpoint === l) return;
					const o = (l in n ? n[l] : void 0) || this.originalParams,
						d = D(this, i),
						c = D(this, o),
						p = i.enabled;
					d && !c
					...
					u && s && this.changeDirection(), f(this.params, o);
					const m = this.params.enabled;
					Object.assign(this, {
				...
				getBreakpoint: (e, t = "window", s) => {
					...
					const i = r(),
						...
						l = Object.keys(e).map((e) => {
							if ("string" == typeof e && 0 === e.indexOf("@")) {
								const t = Number.parseFloat(e.substr(1));
					...
					l.sort(
						(e, t) =>
							Number.parseInt(e.value, 10) - Number.parseInt(t.value, 10),
					);
					for (let e = 0; e < l.length; e += 1) {
			...
			checkOverflow: {
				checkOverflow: function () {
					const { isLocked: t, params: s } = this,
						{ slidesOffsetBefore: a } = s;
					if (a) {
						const t = this.slides.length - 1,
							s = this.slidesGrid[t] + this.slidesSizesGrid[t] + 2 * a;
						this.isLocked = this.size > s;
					} else this.isLocked = 1 === this.snapGrid.length;
					!0 === s.allowSlideNext && (this.allowSlideNext = !this.isLocked),
						!0 === s.allowSlidePrev && (this.allowSlidePrev = !this.isLocked),
	...
	class H {
		constructor(...e) {
			...
			(this.__swiper__ = !0),
				(this.support = y()),
				(this.device = E({ userAgent: s.userAgent })),
				(this.browser = T()),
				(this.eventsListeners = {}),
				(this.eventsAnyListeners = []),
				void 0 === this.modules && (this.modules = []),
				s.modules &&
					Array.isArray(s.modules) &&
					this.modules.push(...s.modules);
			const i = {};
			...
			return (
				(this.params = f({}, r, X, s)),
				(this.originalParams = f({}, this.params)),
				(this.passedParams = f({}, s)),
				this.params &&
					this.params.on &&
					Object.keys(this.params.on).forEach((e) => {
						this.on(e, this.params.on[e]);
					}),
				this.params && this.params.onAny && this.onAny(this.params.onAny),
				(this.$ = d),
				Object.assign(this, {
					...
					isVertical: () => "vertical" === this.params.direction,
					activeIndex: 0,
					realIndex: 0,
					isBeginning: !0,
					isEnd: !1,
					...
					touchEventsData: {
						...
						focusableElements: this.params.focusableElements,
						lastClickTime: u(),
						clickTimeout: void 0,
						velocities: [],
		...
		emitContainerClasses() {
			...
			const t = this.el.className
				...
				.filter(
					(t) =>
						0 === t.indexOf("swiper") ||
						0 === t.indexOf(this.params.containerModifierClass),
				);
		...
		mount(e) {
			...
			if (0 === r.length && this.params.createElements) {
				const e = a().createElement("div");
				(r = d(e)),
					(e.className = this.params.wrapperClass),
					s.append(e),
					s.children(`.${this.params.slideClass}`).each((e) => {
						r.append(e);
					});
			}
			return (
		...
		destroy(e = !0, t = !0) {
			const { params: a, $el: i, $wrapperEl: r, slides: n } = this;
			return (
				void 0 === this.params ||
					this.destroyed ||
					...
					this.emit("destroy"),
					Object.keys(this.eventsListeners).forEach((e) => {
						this.off(e);
					}),
					!1 !== e &&
						((this.$el[0].swiper = null),
						((e) => {
							const t = e;
	...
	function R(e) {
		const { $wrapperEl: s, params: a } = this;
		if ((a.loop && this.loopDestroy(), "object" == typeof e && "length" in e))
			for (let t = 0; t < e.length; t += 1) e[t] && s.append(e[t]);
		else s.append(e);
		a.loop && this.loopCreate(), a.observer || this.update();
	}
	function j(e) {
		const { params: s, $wrapperEl: a, activeIndex: i } = this;
	...
	function q(e, t) {
		const { $wrapperEl: a, params: i, activeIndex: r } = this;
		let n = r;
		i.loop &&
			((n -= this.loopedSlides),
			this.loopDestroy(),
	...
	function V(e) {
		...
		s.loop &&
			((r -= this.loopedSlides),
			this.loopDestroy(),
			(this.slides = a.children(`.${s.slideClass}`)));
		let n,
			l = r;
		if ("object" == typeof e && "length" in e) {
			for (let s = 0; s < e.length; s += 1)
	...
	function F() {
		const t = [];
		for (let s = 0; s < this.slides.length; s += 1) t.push(s);
		this.removeSlide(t);
	}
	function _(e) {
		const {
					...
					prependSlide: (t) => {
						...
						if (e.params.virtual.cache) {
							...
							Object.keys(t).forEach((e) => {
								...
								i &&
									a.attr("data-swiper-slide-index", Number.parseInt(i, 10) + 1),
									(s[Number.parseInt(e, 10) + r] = a);
							}),
								(e.virtual.cache = s);
						}
						i(!0), e.slideTo(a, 0);
				...
				(s[0].style.width = ""),
					...
					(l =
						"auto" === e.params.scrollbar.dragSize
							? o * c
							: Number.parseInt(e.params.scrollbar.dragSize, 10)),
					e.isHorizontal()
		...
		({ swiper: e, extendParams: t, on: s }) => {
			...
			const a = (t, s) => {
					...
					if (
						...
						(l =
							l.indexOf("%") >= 0
								? Number.parseInt(l, 10) * s * r + "%"
								: l * s * r + "px"),
						(o =
							o.indexOf("%") >= 0
								? Number.parseInt(o, 10) * s + "%"
								: o * s + "px"),
						null != p)
					) {
		...
		({ swiper: e, extendParams: t, on: s }) => {
			...
			function v() {
				...
				const i = e.$wrapperEl,
					r =
						i.attr("id") ||
						`swiper-wrapper-${((e = 16) => "x".repeat(e).replace(/x/g, () => Math.round(16 * Math.random()).toString(16)))(16)}`,
				...
				e.slides.each((s, a) => {
					const i = d(s),
						r = e.params.loop
							? Number.parseInt(i.attr("data-swiper-slide-index"), 10)
							: a;
					c(
						i,
						t.slideLabelMessage
				...
				_({
					...
					setTranslate: () => {
						...
						for (let e = 0; e < a.length; e += 1) {
							const t = a.eq(e);
							let s = e;
							u && (s = Number.parseInt(t.attr("data-swiper-slide-index"), 10));
							let i = 90 * s,
								r = Math.floor(i / 360);
				...
				_({
					...
					setTranslate: () => {
						...
						for (let e = 0, t = a.length; e < t; e += 1) {
							...
							"string" == typeof m &&
								-1 !== m.indexOf("%") &&
								(m = (Number.parseFloat(r.stretch) / 100) * s);
							let f = n ? 0 : m * l,
								g = n ? m * l : 0,
